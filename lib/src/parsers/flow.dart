...
class Flow extends GrammarDefinition {
  ...
  Parser circleShape() => (string('((') & ref0(textContent) & string('))')).map(
        (values) => {
  ...
  Parser cylinderShape() =>
      (string('[(') & ref0(textContent) & string(')]')).map(
        (values) => {
  ...
  Parser diamondShape() => (char('{') & ref0(textContent) & char('}')).map(
        (values) => {
  ...
  Parser flowDocument() => (ref0(newline).optional() &
              ref0(graphConfig) &
              ref0(statements) &
              ref0(whitespace))
          .map((values) {
  ...
  Parser graphConfig() => (ref0(spaces).optional() &
          ref0(graphKeyword) &
          ref0(directionSpec) &
          ref0(newline))
  ...
  Parser nodeId() => (letter() | digit() | pattern('_-')).plus().flatten();
  ...
  Parser nodeStatement() => (ref0(spaces) &
              ref0(nodeId) &
  ...
  Parser rectShape() => (char('[') & ref0(textContent) & char(']')).map(
        (values) => {
  ...
  Parser roundShape() => (char('(') & ref0(textContent) & char(')')).map(
        (values) => {
  ...
  Parser statement() =>
      ref0(commentLine) | ref0(nodeStatement) | ref0(emptyLine);
  ...
  Parser textContent() =>
      pattern('^][)(}{').plus().flatten().map((s) => s.trim());
